...
**필요한 배경**: 토큰·컨텍스트 윈도우·시스템 프롬프트([000](000_absolute_basics.md) 6~7절),
그리고 [04](04_agent_loop.md)의 대화 루프.

**비유**: LLM 호출은 매번 **서류 묶음 전체를 다시 제출**하는 일입니다. 그런데 심사관
(LLM 서버)은 "지난번과 앞부분이 토시라도 같으면" 심사비를 크게 깎아 줍니다
(**프롬프트 캐싱**). 그래서 서류는 세 묶음으로 나눕니다:
- 절대 안 바뀌는 표지와 규정집(stable — 시스템 프롬프트),
- 가끔 갱신되는 첨부자료(context),
- 매번 바뀌는 최신 보고(volatile).
그리고 서류가 책상(컨텍스트 윈도우)을 넘치면 가운데 부분을 요약본으로 갈아끼우는
것이 **컨텍스트 압축**입니다.

**학습 목표**: "프롬프트 캐싱은 왜 신성한가"를 비용 숫자 감각으로 설명할 수 있고,
3계층 프롬프트 구조와 압축의 동작 조건·범위를 말할 수 있게 됩니다.
...
**핵심 요약**
- 프롬프트 캐싱은 "같은 앞부분 재사용 시 할인"이므로, 대화 도중 시스템 프롬프트·과거 메시지를 바꾸면 비용이 몇 배로 늘어난다 — 유일한 예외는 압축.
- 프롬프트는 stable(불변) / context(준정적) / volatile(매번 갱신) 3계층으로 나눠 캐시 안정성을 지킨다.
- 압축은 토큰 한도 근접 시 오래된 구간을 요약으로 교체하고, 세션을 분할해 원본을 보존한다([06](06_state.md)).
- 컨텍스트 엔진은 ABC로 추상화되어 플러그인으로 교체 가능하다.

**점검 질문**
1. 시스템 프롬프트를 대화 중간에 한 글자라도 바꾸면 왜 비용이 늘어나는가?
2. stable/context/volatile 각각에 들어가는 내용의 예를 하나씩 들 수 있는가?
3. 압축은 캐시를 깨는데도 왜 허용되는 유일한 예외인가? (힌트: 안 하면 아예 호출 불가)
4. 압축할 때 "최근 메시지"는 왜 요약 대상에서 제외하는가?
