...
**필요한 배경**: [04](04_agent_loop.md)의 대화 루프 — 특히 "LLM이 도구 호출 JSON을
내려보내면 프로그램이 실행해 준다"는 흐름.

**비유**: 도구 계층은 회사의 **공구방 + 접수창구**입니다.
- 각 공구(도구 모듈)는 입사할 때 스스로 명부에 이름을 적습니다(**자기 등록**).
- 접수창구(**레지스트리**)는 "terminal 공구 쓰게 해줘"라는 요청을 받아 명부에서 찾아
  실행해 줍니다(**디스패치**).
- 명부에 올릴 수 있는 공구 수는 엄격히 제한됩니다. 모든 공구 설명서가 **매번 LLM에게
  전송되어 토큰 비용을 발생**시키기 때문입니다 — 이게 "좁은 허리(narrow waist)" 설계의 이유입니다.

**학습 목표**: 새 도구 파일이 어떻게 발견·등록·노출·실행되는지 4단계로 설명할 수 있고,
"왜 아무 기능이나 코어 도구로 추가하면 안 되는지"를 비용 관점에서 논증할 수 있게 됩니다.
...
**핵심 요약**
- 도구는 `tools/` 하위 모듈이 import 시점에 `registry.register(...)`로 스스로 등록하고, `discover_builtin_tools`가 폴더를 훑어 자동 발견한다.
- LLM에게 노출되는 도구 목록은 toolset·`check_fn`(서비스 게이팅)으로 좁혀진다 — 모든 도구 스키마는 매 API 호출마다 비용이다.
- 실행은 `handle_function_call` → `registry.dispatch(name, args)` 경로로 일어난다.
- 새 기능은 "기존 코드 확장 → CLI+스킬 → 게이팅 도구 → 플러그인 → MCP → 코어 도구(최후)" 순서로 검토한다(footprint ladder).

**점검 질문**
1. 새 파일을 `tools/`에 넣기만 하면 도구가 생기는 메커니즘을 단계별로 설명할 수 있는가?
2. `check_fn`은 어떤 문제를 푸는가? (힌트: 설정 안 된 서비스의 도구가 노출되면?)
3. "코어 도구 하나 추가"가 왜 모든 사용자의 매 호출 비용을 늘리는가?
4. terminal + file 도구만으로 해결 가능한 일에 새 도구를 만들면 안 되는 이유는?
