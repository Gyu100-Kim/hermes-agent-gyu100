...
**필요한 배경**: 데이터베이스와 표(테이블) 개념([000](000_absolute_basics.md) 9절).
SQL 문법을 몰라도 예제마다 풀어 설명하므로 따라올 수 있습니다.

**비유**: 이 계층은 회사의 **문서보관실**입니다.
- 대화 한 건 = 서류철 하나(sessions), 메시지 한 줄 = 서류 한 장(messages).
- WAL 모드 = "수정 중에도 열람은 계속 가능한 보관 규칙" — 쓰는 사람이 서류를 잠그지 않음.
- FTS5 = 보관실 뒷방의 **색인 카드 캐비닛**. "그 얘기 언제 했더라?"를 서류 전체를
  뒤지지 않고 단어 색인으로 바로 찾습니다.
- `parent_session_id` = 서류철이 너무 두꺼워지면 요약본으로 새 서류철을 파고, 원본으로
  거슬러 올라갈 수 있는 연결 고리.

**학습 목표**: "왜 파일 대신 SQLite인가", "WAL·FTS5가 각각 무엇을 해결하는가",
"압축 시 세션이 어떻게 분할되는가"를 설명할 수 있게 됩니다.
...
**핵심 요약**
- 상태는 파일 하나짜리 SQLite DB(`hermes_state.py`의 `SessionDB`)에 저장된다 — 서버 설치 없이 트랜잭션·동시성·검색을 얻는다.
- 핵심 테이블은 sessions(대화)와 messages(메시지)다.
- WAL 모드는 읽기와 쓰기가 서로를 막지 않게 하고, FTS5 + 트라이그램 인덱스는 한글 포함 전문 검색을 가능하게 한다.
- 컨텍스트 압축 시 `parent_session_id`로 새 세션을 분할해 원본 이력을 보존한다.

**점검 질문**
1. 그냥 JSON 파일에 대화를 저장하면 어떤 문제가 생길까? (동시 쓰기, 부분 검색)
2. WAL 모드가 없다면 게이트웨이처럼 여러 작업이 동시에 돌 때 무슨 일이 생기는가?
3. FTS5의 역색인은 일반 `LIKE '%단어%'` 검색과 무엇이 다른가?
4. 한국어 검색을 위해 트라이그램 인덱스가 추가로 필요한 이유는? (힌트: 띄어쓰기와 토큰화)
5. 압축된 세션에서 예전 원본 대화를 찾아가려면 어떤 컬럼을 따라가면 되는가?
