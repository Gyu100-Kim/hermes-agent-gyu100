...
**필요한 배경**: 토큰과 컨텍스트 윈도우([../000_absolute_basics.md](../000_absolute_basics.md) 6절).

**비유**: 컨텍스트 윈도우는 회의실 화이트보드입니다. 회의가 길어져 보드가 꽉 차면,
오래된 내용을 지우기 전에 한쪽 구석에 **요약본**을 적어 둡니다. 무엇을 지우고(오래된
중간 부분), 무엇을 남기며(최근 내용과 규칙), 요약은 언제 하는가(보드가 거의 찼을 때)
— 이 세 질문이 압축 설계의 전부입니다. 단, 지우기는 비용(캐시 무효화)이 들기 때문에
가능한 한 미룹니다.

**학습 목표**: 압축이 필요해지는 조건, Lost-in-the-Middle 같은 근거 연구, Hermes의
압축 시점·범위·세션 분할을 연결해 설명할 수 있게 됩니다.
...
1. 컨텍스트가 길어질수록 생기는 문제 두 가지(비용, 품질)는 각각 무엇인가?
2. Lost-in-the-Middle 현상은 압축 설계에 어떤 시사점을 주는가?
3. 압축을 "매 턴"이 아니라 "한도 근접 시에만" 하는 이유는? (힌트: 프롬프트 캐시)
4. 압축 후 원본 대화는 어디에 어떻게 보존되는가?
