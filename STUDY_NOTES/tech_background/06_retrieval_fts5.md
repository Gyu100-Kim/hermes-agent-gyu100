...
수단인 SQLite ([용어사전](../../dict/06_state_retrieval.md#sqlite)) **FTS5 ([용어사전](../../dict/06_state_retrieval.md#fts5))**를 다룹니다. 정보검색(IR)의 기초 개념(역색인 ([용어사전](../../dict/06_state_retrieval.md#inverted-index)), 토크나이저 ([용어사전](../../dict/06_state_retrieval.md#tokenizer)),
BM25 ([용어사전](../../dict/06_state_retrieval.md#bm25)))부터, RAG와의 관계, CJK(한중일) 텍스트 검색의 특수한 어려움과 트라이그램
해법까지 하위 개념을 상세히 풀고, IR의 오랜 역사와 근간 문헌을 정리한 뒤 Hermes의
세션 검색 ([용어사전](../../dict/06_state_retrieval.md#session-search)) 구현과 연결합니다.
...
**필요한 배경**: 데이터베이스와 테이블 개념([../000_absolute_basics.md](../000_absolute_basics.md) 9절),
그리고 [../06_state.md](../06_state.md)의 SQLite 스키마.

**비유**: 책 뒤의 **찾아보기(색인)**를 떠올리세요. "프롬프트"라는 단어가 어느 쪽에
나오는지 찾으려면, 책을 처음부터 다 읽는 대신(LIKE 검색) 색인에서 바로 쪽 번호를
찾습니다(역색인). FTS5는 SQLite에 내장된 색인 자동 관리자이고, 트라이그램은
띄어쓰기가 없는 한국어도 3글자 단위로 색인해 찾을 수 있게 하는 보조 장치입니다.

**학습 목표**: 역색인·BM25·토큰화의 역할을 구별하고, Hermes 세션 검색이 왜 FTS5 +
트라이그램 이중 구성인지 설명할 수 있게 됩니다.
...
찾아서 가져오기"** — 이것이 **검색 증강 ([용어사전](../../dict/06_state_retrieval.md#retrieval))(retrieval)** 입니다.

- **전문 검색 ([용어사전](../../dict/06_state_retrieval.md#fts))(full-text search, FTS)**: 문서 안의 단어를 미리 **색인(index)** 해
...
- **unicode61 ([용어사전](../../dict/06_state_retrieval.md#unicode61))** (FTS5 기본): 공백/문장부호 기준으로 단어를 나눔. 영어처럼 공백으로
  단어가 구분되는 언어에 적합합니다.
...
- **벡터 검색 ([용어사전](../../dict/06_state_retrieval.md#vector-search))**: 텍스트를 임베딩(숫자 벡터)으로 바꿔 **의미적 유사도**로 찾음.
  단어가 달라도("자동차"↔"차량") 찾을 수 있지만, 임베딩 계산·저장 비용이 들고
  정확한 문자열(에러 메시지, 함수명) 매칭에는 오히려 약합니다.
- **하이브리드 검색 ([용어사전](../../dict/06_state_retrieval.md#hybrid-search))**: BM25(정확 매칭에 강함) + 벡터(의미 매칭에 강함)를 병행하고
...
1. **1970s~ — 정보검색(IR)의 정립**: 역색인, TF-IDF ([용어사전](../../dict/06_state_retrieval.md#tf-idf)), 확률적 랭킹 모델 등 검색의
...
5. **2020 — RAG** (Lewis et al.): "검색 + 생성" 결합을 정식화. LLM ([용어사전](../../dict/01_llm_basics.md#llm)) 시대의 검색
   르네상스를 엽니다.
...
1. 역색인이 LIKE 검색보다 빠른 이유를 자료구조 관점에서 설명할 수 있는가?
2. BM25는 무엇을 기준으로 문서의 순위를 매기는가? (힌트: 빈도와 희소성)
3. 한국어 검색에 트라이그램 토크나이저가 필요한 이유는?
4. 임베딩 기반 의미 검색 대신 FTS5를 쓴 Hermes의 설계 판단은 어떤 트레이드오프인가?
