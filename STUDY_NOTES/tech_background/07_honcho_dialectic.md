...
**필요한 배경**: [02_self_improving_agents.md](02_self_improving_agents.md)의 메모리 개념.

**비유**: 단골 카페 직원을 생각해 보세요. 메모장에 "아메리카노 주문함"이라고 사실만 적는
직원(단순 메모리)과, "이 손님은 단맛을 싫어하고 아침엔 연하게 마시는 편"이라고
**해석까지 쌓는** 직원(사용자 모델링)은 다릅니다. Honcho의 변증적(dialectic) API는
"이 사용자라면 무엇을 좋아할까?" 같은 질문에 추론된 답을 주는, 후자 방식의 메모리
서비스입니다.

**학습 목표**: "사실 저장"과 "사용자 모델링"의 차이, 변증적 API의 의미, Hermes에서
Honcho가 플러그인으로 붙는 방식을 설명할 수 있게 됩니다.
...
1. 단순 사실 저장 메모리와 사용자 모델링의 차이는 무엇인가?
2. "변증적(dialectic)"이라는 이름은 어떤 동작 방식을 가리키는가?
3. Hermes에서 메모리 provider가 코어가 아니라 플러그인인 이유는?
4. 사용자 모델링이 틀렸을 때의 위험과 완화책은?
