...
**필요한 배경**: 01~09 문서의 큰 그림. 이 문서는 본관 주변의 **별관들**을 둘러보는
투어입니다.

**비유**: 지금까지 본 것이 회사의 본관(코어·도구·보관실·교환원)이었다면, 여기서는:
- **cron** = 예약 업무 담당 비서("매일 아침 9시에 보고서 준비").
- **plugins** = 외부 협력업체 사무실 — 본관 설계도를 건드리지 않고 기능을 덧붙임.
- **ACP 어댑터** = 코드 에디터(Zed 등)와 대화하는 전용 통역사.
- **TUI/데스크톱/웹** = 같은 본관을 보여주는 서로 다른 유리창(인터페이스).
- **실행 환경 추상화** = 위험한 작업을 본관 밖 안전 실험실(Docker 등)에서 하게 하는 장치.

**학습 목표**: 각 부가 서브시스템이 "본관(코어)과 어떤 경계선으로 만나는지"를 한
문장씩 말할 수 있게 됩니다.
...
**핵심 요약**
- cron은 스케줄 계산(scheduler)·작업 정의(jobs)·실행 기록(executions)으로 분리된 예약 작업 시스템이다.
- plugins는 코어가 제공하는 ABC·훅 안에서만 동작하며, 코어 파일을 직접 건드리지 않는다.
- ACP 어댑터는 에디터와 에이전트를 표준 프로토콜(JSON-RPC)로 잇고, TUI·데스크톱·웹은 같은 코어의 다른 얼굴이다.
- 실행 환경 추상화(`tools/environments/`)는 local/Docker/SSH/Modal/Daytona/Singularity를 같은 인터페이스로 다룬다.

**점검 질문**
1. cron 작업이 "다음 실행 시각"을 어떻게 계산하는가? (힌트: croniter)
2. 플러그인이 코어 파일을 수정하면 안 되는 이유는?
3. 터미널 백엔드를 추상화해 둔 덕분에 가능해지는 일은 무엇인가?
4. TUI와 데스크톱 앱이 각각 다른 언어/프레임워크를 쓰면서도 같은 코어를 공유할 수 있는 이유는?
