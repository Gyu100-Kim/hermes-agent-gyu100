...
**필요한 배경**: 파일/폴더 개념, 파이썬 파일(`.py`)이 무엇인지 정도면 충분합니다.
하나도 모르겠다면 먼저 [000_absolute_basics.md](000_absolute_basics.md)를 읽고 오세요.

**비유로 먼저 잡는 그림**: 이 저장소를 하나의 **회사 건물**이라고 생각해 보세요.

- 건물의 중앙 엔진실(= 에이전트 코어, `run_agent.py`·`agent/`)은 하나입니다.
- 건물에는 출입구가 여러 개입니다(= CLI·TUI·메신저 게이트웨이·데스크톱 앱).
  어느 문으로 들어와도 같은 엔진실에 닿습니다 — 이게 "하나의 코어, 여러 개의 얼굴" 설계입니다.
- 공구함(= `tools/`), 문서보관실(= SQLite 상태 저장), 외부 협력업체 연결부(= MCP·플러그인)가
  각각 별도 방으로 분리되어 있습니다.

**이 문서의 학습 목표**: 읽고 나면 (1) 최상위 폴더 이름만 보고도 어떤 기능이 어디 있을지
추측할 수 있고, (2) "코어는 좁게, 확장은 가장자리에"라는 이 프로젝트의 핵심 설계
철학을 자기 말로 설명할 수 있게 됩니다.
...
**핵심 요약**
- Hermes는 하나의 에이전트 코어를 CLI·TUI·게이트웨이·데스크톱 등 여러 인터페이스가 공유하는 구조다.
- 최상위에는 역사적으로 커진 단일 파일 모듈(`run_agent.py`, `cli.py` 등)과 역할별 패키지(`agent/`, `tools/`, `gateway/` 등)가 공존한다.
- 주 언어는 Python(3.11~3.13)이고, TUI는 TypeScript/React(Ink), 재현 가능한 환경은 Nix가 보조한다.
- 설계 철학: 코어(좁은 허리)는 최소로 유지하고, 새 기능은 플러그인·스킬·게이트웨이 어댑터 같은 가장자리에 붙인다.

**점검 질문** (답할 수 있으면 다음 문서로)
1. 사용자가 텔레그램으로 메시지를 보내든 터미널에서 `hermes`를 실행하든, 결국 어떤 코드가 응답을 만드는가?
2. `tools/`와 `agent/`는 역할이 어떻게 다른가?
3. "하나의 코어, 여러 개의 얼굴" 구조가 주는 이점은 무엇인가?
4. 새로운 기능을 추가한다면 왜 코어보다 가장자리(플러그인/스킬)가 먼저 고려되는가?
