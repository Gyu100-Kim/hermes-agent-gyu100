...
발견하면 그것을 **스킬(SKILL.md ([용어사전](../dict/05_memory_self_improvement.md#skill-md)))** 로 저장하고, 사용자의 취향·사실은 **메모리
(MEMORY.md/USER.md)** 로 기억합니다. 그리고 백그라운드 **큐레이터 ([용어사전](../dict/05_memory_self_improvement.md#curator))(curator)** 가
쌓인 스킬을 주기적으로 정비합니다.
...
4. **메모리 매니저 ([용어사전](../dict/05_memory_self_improvement.md#memory-manager))** (`agent/memory_manager.py`) — 기억 provider 오케스트레이션 ([용어사전](../dict/02_agent_core.md#orchestration))
...
**필요한 배경**: [04](04_agent_loop.md)의 대화 루프와 [07](07_prompt_context.md)의
프롬프트 구조. "LLM은 세션이 끝나면 다 잊는다"는 전제를 기억하세요.

**비유**: LLM은 매일 기억이 초기화되는 신입사원과 같습니다. 그래서 Hermes는
회사 차원의 기록 체계를 둡니다:
- **스킬(SKILL.md)** = 업무 매뉴얼 문서. "이런 상황엔 이 순서로 하라"는 절차서.
- **/learn** = 오늘 겪은 시행착오를 매뉴얼로 정리해 남기는 회고 회의.
- **큐레이터(curator)** = 매뉴얼 서가를 주기적으로 정리하는 사서(중복 병합, 낡은 것 보관).
- **메모리 매니저** = 개인 수첩(사용자 선호·사실)을 관리하는 창구.
이렇게 "경험 → 문서화 → 재사용"의 고리가 돌면, 같은 실수를 반복하지 않는
**자기개선 루프**가 됩니다 (배경 이론은 [tech_background/02](tech_background/02_self_improving_agents.md)).

**학습 목표**: 스킬의 파일 구조와 발동 방식, /learn→큐레이터→메모리로 이어지는
학습 파이프라인을 설명할 수 있게 됩니다.
...
- **점진적 공개 ([용어사전](../dict/05_memory_self_improvement.md#progressive-disclosure))(progressive disclosure)**: 시스템 프롬프트에는 스킬의 **설명 한
...
- **별도의 정제 엔진도, 모델-도구 발자국도 없습니다**(18-22행): 에이전트가 자기
  도구로 처리하므로 local/Docker/원격 백엔드에서 동일하게 동작하고, CLI/게이트웨이 ([용어사전](../dict/07_gateway_interfaces.md#gateway))/
  대시보드의 모든 `/learn` 진입점이 `build_learn_prompt` 하나를 공유합니다.

> 이것은 [05](05_tools.md)의 "Footprint Ladder ([용어사전](../dict/11_design_principles.md#footprint-ladder))"에서 2번 rung(CLI 명령 + 스킬)을
...
> "배운 것"이 결국 시스템 프롬프트 ([용어사전](../dict/01_llm_basics.md#system-prompt)) 색인에 들어가므로, 그 형식이 곧 비용과 라우팅
> 품질을 좌우합니다.
...
- **비활성 트리거**(3-7행): 별도 cron 데몬 없이, 에이전트가 idle이고 마지막 큐레이터
  실행이 `interval_hours` 이전이면 `maybe_run_curator()`가 **포크된 AIAgent ([용어사전](../dict/02_agent_core.md#aiagent))**를 띄워
  리뷰합니다.
...
> [07](07_prompt_context.md)의 "신성한 캐시"가 깨집니다. 그래서 별도의 보조 모델 ([용어사전](../dict/01_llm_basics.md#auxiliary-model))/
...
- **외부 provider는 한 번에 하나만**(6-8행): 두 번째 외부 provider 등록은 경고와
  함께 거부됩니다. 도구 스키마 ([용어사전](../dict/03_tool_system.md#tool-schema)) 비대와 백엔드 충돌을 막기 위함(좁은 허리).
...
- 기본 내장 provider는 `MEMORY.md`/`USER.md` 파일 기반이고, 플러그인으로
  Honcho ([용어사전](../dict/05_memory_self_improvement.md#honcho))/mem0/supermemory 등을 붙일 수 있습니다(`plugins/memory/`).
...
- 노드: 베이스가 아닌 **학습/프로필 ([용어사전](../dict/07_gateway_interfaces.md#profile)) 스킬**과 `MEMORY.md`/`USER.md`의 **기억 조각**을
...
**핵심 요약**
- 스킬은 frontmatter + 본문 절차로 된 SKILL.md 문서다. 목록만 프롬프트에 실리고, 본문은 필요할 때 전체를 읽는다(부분 읽기 금지).
- /learn은 현재 세션의 경험을 새 스킬 또는 기존 스킬 개선으로 변환한다.
- 큐레이터는 스킬 서가를 주기적으로 정비(중복 병합·아카이브)하고, 메모리 매니저는 사용자 사실·선호를 통합 관리한다.
- 학습 그래프는 "무엇을 배웠는가"를 시각화해 자기개선을 관찰 가능하게 한다.

**점검 질문**
1. 스킬 본문을 페이지 단위로 잘라 읽게 하면(페이지네이션) 왜 위험한가?
2. /learn이 없다면 에이전트의 경험은 세션 종료 후 어떻게 되는가?
3. 큐레이터가 오래된 스킬을 삭제가 아니라 아카이브하는 이유는?
4. 스킬과 메모리는 각각 어떤 종류의 기억(절차 vs 사실)을 담당하는가?

다음 문서에서는 스케줄러·플러그인·ACP ([용어사전](../dict/08_protocols.md#acp))·프론트엔드 등 **부가 서브시스템**을 봅니다.
