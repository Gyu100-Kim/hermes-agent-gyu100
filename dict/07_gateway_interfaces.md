...
**영문**: Gateway · **분류**: [게이트웨이·인터페이스](README.md#분류content-class)
...
**상위 개념(더 특수)**: [딜리버리 (출력 라우팅)](#delivery) · [게이트웨이 세션](#gateway-session) · [플랫폼 어댑터](#platform-adapter) · [슬래시 명령](#slash-command)
...
**영문**: Platform Adapter · **분류**: [게이트웨이·인터페이스](README.md#분류content-class)
...
**하위 개념(더 일반)**: [게이트웨이](#gateway)

**상위 개념(더 특수)**: [API 서버 어댑터](#api-server) · [웹훅](#webhook)
...
**영문**: Delivery · **분류**: [게이트웨이·인터페이스](README.md#분류content-class)
...
**영문**: Unified Session · **분류**: [게이트웨이·인터페이스](README.md#분류content-class)

여러 플랫폼(예: Telegram과 CLI)에서 온 대화를 하나의 세션으로 이어주는 기능. 어디서 말을 걸어도 같은 맥락이 유지됩니다.

**하위 개념(더 일반)**: [게이트웨이 세션](#gateway-session)
...
**영문**: Slash Command · **분류**: [게이트웨이·인터페이스](README.md#분류content-class)
...
**영문**: HermesCLI · **분류**: [게이트웨이·인터페이스](README.md#분류content-class)

터미널 대화형 인터페이스의 오케스트레이터(`cli.py`). `hermes` 명령의 진입점이며 [REPL](#repl) 루프를 제공합니다.

**상위 개념(더 특수)**: [REPL](#repl) · [설정 마법사](11_design_principles.md#setup-wizard) · [스킨 (CLI 테마)](11_design_principles.md#skin)
...
**영문**: REPL · **분류**: [게이트웨이·인터페이스](README.md#분류content-class)

읽기(Read)-평가(Eval)-출력(Print) 반복(Loop)의 약자. 터미널에서 한 줄씩 입력하고 즉시 응답을 받는 대화형 실행 방식입니다.

**하위 개념(더 일반)**: [HermesCLI](#hermes-cli)
...
**영문**: TUI · **분류**: [게이트웨이·인터페이스](README.md#분류content-class)

React 기반 Ink 프레임워크로 만든 터미널 그래픽 인터페이스(`ui-tui/`). 일반 CLI보다 풍부한 표시(패널, 진행 표시)를 제공합니다.

**상위 개념(더 특수)**: [TUI 게이트웨이](#tui-gateway)
...
**영문**: Desktop App · **분류**: [게이트웨이·인터페이스](README.md#분류content-class)
...
**영문**: Web Dashboard · **분류**: [게이트웨이·인터페이스](README.md#분류content-class)
...
**영문**: Profile · **분류**: [게이트웨이·인터페이스](README.md#분류content-class)
...
**상위 개념(더 특수)**: [HERMES_HOME](#hermes-home)
...
**영문**: HERMES_HOME · **분류**: [게이트웨이·인터페이스](README.md#분류content-class)

Hermes의 상태(설정, DB, 스킬, 로그)가 저장되는 홈 디렉토리. [프로필](#profile)마다 다른 경로를 가집니다.

**하위 개념(더 일반)**: [프로필](#profile)
...
**영문**: Webhook · **분류**: [게이트웨이·인터페이스](README.md#분류content-class)
...
**하위 개념(더 일반)**: [플랫폼 어댑터](#platform-adapter)
...
**영문**: API Server Adapter · **분류**: [게이트웨이·인터페이스](README.md#분류content-class)
