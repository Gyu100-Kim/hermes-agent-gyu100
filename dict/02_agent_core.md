...
이 문서는 **에이전트 코어·대화 루프** 범주의 용어 26개를 다룹니다. 설명 속 파란 링크를 누르면 해당 용어 항목으로 이동하며, 각 항목 끝의 "이 용어를 참조하는 항목"으로 되돌아올 수 있습니다.
...
**하위 개념**: [AIAgent 클래스](#aiagent) · [Mixture-of-Agents (MoA)](#moa) · [ReAct 패턴](#react) · [자기개선 에이전트](05_memory_self_improvement.md#self-improving-agent) · [서브에이전트 / 위임](#subagent) · [도구 호출 (함수 호출)](#tool-calling)
...
**영문**: AIAgent (run_agent.py) · **범주**: 에이전트 코어·대화 루프
...
**영문**: Tool Calling / Function Calling · **범주**: 에이전트 코어·대화 루프
...
**하위 개념**: [메시지 프로토콜](#message-protocol) · [병렬 도구 호출](#parallel-tool-calls) · [도구](03_tool_system.md#tool) · [도구 호출 루프](#tool-calling-loop)

**관련 용어**: [도구 스키마](03_tool_system.md#tool-schema) · [디스패치](03_tool_system.md#dispatch) · [ReAct 패턴](#react)

**이 용어를 참조하는 항목**: [에이전트](#agent) · [에이전트형 RAG](06_state_retrieval.md#agentic-rag) · [환각](01_llm_basics.md#hallucination) · [LLM (대규모 언어 모델)](01_llm_basics.md#llm) · [ReAct 패턴](#react)
...
**영문**: Tool-Calling Loop · **범주**: 에이전트 코어·대화 루프

모델 호출 → 도구 실행 → 결과 반환 → 다시 모델 호출을 반복하는 에이전트의 심장 박동. 모델이 도구 호출 없이 최종 답을 내놓거나 [반복 예산](#iteration-budget)이 소진되면 종료됩니다.

**상위 개념**: [도구 호출 (함수 호출)](#tool-calling)

**하위 개념**: [반복 예산](#iteration-budget) · [역할 교대 불변식](#role-alternation) · [run_conversation 루프](#run-conversation) · [턴](#turn)
...
**영문**: run_conversation · **범주**: 에이전트 코어·대화 루프
...
**상위 개념**: [도구 호출 루프](#tool-calling-loop)

**하위 개념**: [재시도와 폴백](#retry-fallback)
...
**영문**: Grace Call · **범주**: 에이전트 코어·대화 루프

[반복 예산](#iteration-budget)이 소진됐을 때, 어중간하게 끊기지 않도록 '마무리 답변을 만들기 위한 마지막 한 번의 호출'을 허용하는 장치.

**상위 개념**: [반복 예산](#iteration-budget)

**관련 용어**: [run_conversation 루프](#run-conversation)

**이 용어를 참조하는 항목**: [반복 예산](#iteration-budget) · [run_conversation 루프](#run-conversation)
...
**영문**: ReAct (Reason + Act) · **범주**: 에이전트 코어·대화 루프
...
**영문**: Role Alternation · **범주**: 에이전트 코어·대화 루프
...
**영문**: Parallel Tool Calls · **범주**: 에이전트 코어·대화 루프
...
**영문**: Credential Pool · **범주**: 에이전트 코어·대화 루프
...
**영문**: Mixture-of-Agents · **범주**: 에이전트 코어·대화 루프
...
**하위 개념**: [제안자/종합자](#proposer-aggregator)

**관련 용어**: [제안자/종합자](#proposer-aggregator) · [앙상블](#ensemble) · [Mixture-of-Experts (MoE)](#moe)

**이 용어를 참조하는 항목**: [앙상블](#ensemble) · [Mixture-of-Experts (MoE)](#moe) · [LLM 제공자](#provider) · [자기일관성](#self-consistency) · [슬래시 명령](07_gateway_interfaces.md#slash-command)
...
**영문**: Self-Consistency · **범주**: 에이전트 코어·대화 루프
...
**영문**: LLM-as-a-Judge · **범주**: 에이전트 코어·대화 루프
...
**이 용어를 참조하는 항목**: [제안자/종합자](#proposer-aggregator)
...
**영문**: Mixture-of-Experts · **범주**: 에이전트 코어·대화 루프
...
**영문**: Delegation · **범주**: 에이전트 코어·대화 루프

복잡한 작업을 하위 작업으로 나눠 [서브에이전트](#subagent)에 맡기는 패턴. 컨텍스트를 분리해 메인 대화를 깨끗하게 유지하는 효과도 있습니다. 다중 에이전트 협업에는 [칸반](#kanban)이 사용됩니다.

**상위 개념**: [서브에이전트 / 위임](#subagent)

**하위 개념**: [칸반 (다중 에이전트 보드)](#kanban)

**관련 용어**: [칸반 (다중 에이전트 보드)](#kanban)

**이 용어를 참조하는 항목**: [delegate 도구](03_tool_system.md#delegate-tool) · [반복 예산](#iteration-budget) · [서브에이전트 / 위임](#subagent)
