...
**영문**: Agent · **분류**: [에이전트 코어·대화 루프](README.md#분류content-class) · **최초 등장**: 1995(AI 교과서)·2022(LLM 에이전트)
...
**하위 개념(더 일반·근본)**: [LLM (대규모 언어 모델)](01_llm_basics.md#llm) · [도구 호출 (함수 호출)](#tool-calling)

**상위 개념(이를 활용해 만든 개념)**: [에이전트형 RAG](06_state_retrieval.md#agentic-rag) · [AIAgent 클래스](#aiagent) · [자율성 수준](#autonomy) · [휴먼 인 더 루프](#human-in-the-loop) · [메모리 (에이전트 기억)](05_memory_self_improvement.md#memory) · [다중 에이전트 시스템](#multi-agent) · [자기개선 에이전트](05_memory_self_improvement.md#self-improving-agent) · [자기 성찰](05_memory_self_improvement.md#self-reflection) · [서브에이전트 / 위임](#subagent) · [트리아지 스위퍼](12_subsystems.md#triage-sweeper)

**관련 용어**: [도구 호출 루프](#tool-calling-loop) · [AIAgent 클래스](#aiagent)
...
**영문**: AIAgent (run_agent.py) · **분류**: [에이전트 코어·대화 루프](README.md#분류content-class) · **최초 등장**: 2025
...
**하위 개념(더 일반·근본)**: [에이전트](#agent)

**상위 개념(이를 활용해 만든 개념)**: [배치 러너](12_subsystems.md#batch-runner)

**관련 용어**: [run_conversation 루프](#run-conversation) · [도구 호출 루프](#tool-calling-loop)

**이 용어를 참조하는 항목**: [에이전트](#agent) · [HermesCLI](07_gateway_interfaces.md#hermes-cli) · [run_conversation 루프](#run-conversation)
...
**영문**: Tool Calling / Function Calling · **분류**: [에이전트 코어·대화 루프](README.md#분류content-class) · **최초 등장**: 2023-06
...
**하위 개념(더 일반·근본)**: [LLM (대규모 언어 모델)](01_llm_basics.md#llm) · [도구](03_tool_system.md#tool)

**상위 개념(이를 활용해 만든 개념)**: [에이전트](#agent) · [메시지 프로토콜](#message-protocol) · [병렬 도구 호출](#parallel-tool-calls) · [도구 호출 루프](#tool-calling-loop)

**관련 용어**: [도구 스키마](03_tool_system.md#tool-schema) · [디스패치](03_tool_system.md#dispatch) · [ReAct 패턴](#react)

**이 용어를 참조하는 항목**: [에이전트형 RAG](06_state_retrieval.md#agentic-rag) · [환각](01_llm_basics.md#hallucination) · [LLM (대규모 언어 모델)](01_llm_basics.md#llm) · [ReAct 패턴](#react) · [구조화 출력](01_llm_basics.md#structured-output) · [도구](03_tool_system.md#tool)
...
**영문**: Tool-Calling Loop · **분류**: [에이전트 코어·대화 루프](README.md#분류content-class) · **최초 등장**: 2022-10
...
**하위 개념(더 일반·근본)**: [도구 호출 (함수 호출)](#tool-calling)

**상위 개념(이를 활용해 만든 개념)**: [루프 종료 조건](#agent-loop-termination) · [반복 예산](#iteration-budget) · [역할 교대 불변식](#role-alternation) · [run_conversation 루프](#run-conversation) · [턴](#turn)
...
**영문**: run_conversation · **분류**: [에이전트 코어·대화 루프](README.md#분류content-class) · **최초 등장**: 2025
...
**하위 개념(더 일반·근본)**: [도구 호출 루프](#tool-calling-loop)

**상위 개념(이를 활용해 만든 개념)**: [재시도와 폴백](#retry-fallback)
...
**영문**: Turn · **분류**: [에이전트 코어·대화 루프](README.md#분류content-class) · **최초 등장**: 대화시스템 일반(1990년대)

사용자 메시지 하나에 대해 에이전트가 응답을 완성하기까지의 한 사이클. 한 턴 안에 여러 번의 모델 호출과 도구 실행이 있을 수 있습니다.

**하위 개념(더 일반·근본)**: [도구 호출 루프](#tool-calling-loop)

**상위 개념(이를 활용해 만든 개념)**: [트래젝토리 (실행 궤적)](12_subsystems.md#trajectory) · [턴 격리](11_design_principles.md#turn-isolation)
...
**영문**: Iteration Budget · **분류**: [에이전트 코어·대화 루프](README.md#분류content-class) · **최초 등장**: 2025
...
**상위 개념(이를 활용해 만든 개념)**: [그레이스 콜](#grace-call)
...
**영문**: Grace Call · **분류**: [에이전트 코어·대화 루프](README.md#분류content-class) · **최초 등장**: 2025

[반복 예산](#iteration-budget)이 소진됐을 때, 어중간하게 끊기지 않도록 '마무리 답변을 만들기 위한 마지막 한 번의 호출'을 허용하는 장치.

**하위 개념(더 일반·근본)**: [반복 예산](#iteration-budget)
...
**영문**: ReAct (Reason + Act) · **분류**: [에이전트 코어·대화 루프](README.md#분류content-class) · **최초 등장**: 2022-10
...
**하위 개념(더 일반·근본)**: [프롬프트](01_llm_basics.md#prompt)
...
**영문**: Role Alternation · **분류**: [에이전트 코어·대화 루프](README.md#분류content-class) · **최초 등장**: 2023-03
...
**영문**: Message Protocol (roles) · **분류**: [에이전트 코어·대화 루프](README.md#분류content-class) · **최초 등장**: 2023-03

system/user/assistant/tool 네 가지 역할로 구성되는 대화 형식. 도구 결과는 tool 역할 메시지로 반환되며 [tool_call_id](#tool-call-id)로 어느 호출의 결과인지 짝지어집니다.

**하위 개념(더 일반·근본)**: [도구 호출 (함수 호출)](#tool-calling)

**상위 개념(이를 활용해 만든 개념)**: [tool_call_id](#tool-call-id)
...
**영문**: tool_call_id · **분류**: [에이전트 코어·대화 루프](README.md#분류content-class) · **최초 등장**: 2023-11

모델이 낸 각 도구 호출에 부여되는 고유 식별자. [병렬 도구 호출](#parallel-tool-calls) 시 어떤 결과가 어떤 호출에 대응하는지 구분하는 데 필수입니다.

**하위 개념(더 일반·근본)**: [메시지 프로토콜](#message-protocol)
...
**영문**: Parallel Tool Calls · **분류**: [에이전트 코어·대화 루프](README.md#분류content-class) · **최초 등장**: 2023-11
...
**영문**: Retry & Fallback · **분류**: [에이전트 코어·대화 루프](README.md#분류content-class) · **최초 등장**: 2025
...
**하위 개념(더 일반·근본)**: [run_conversation 루프](#run-conversation)
...
**영문**: LLM Provider · **분류**: [에이전트 코어·대화 루프](README.md#분류content-class) · **최초 등장**: 2023
...
**하위 개념(더 일반·근본)**: [LLM (대규모 언어 모델)](01_llm_basics.md#llm)

**상위 개념(이를 활용해 만든 개념)**: [크리덴셜 풀](#credential-pool) · [모델 메타데이터](#model-metadata) · [Nous Portal](11_design_principles.md#nous-portal)
...
**영문**: Model Metadata · **분류**: [에이전트 코어·대화 루프](README.md#분류content-class) · **최초 등장**: 2025

각 모델의 [컨텍스트 한도](01_llm_basics.md#context-window), 지원 기능(도구 호출, 비전), 가격 등의 정보. 압축 임계값 계산과 기능 분기에 사용됩니다.

**하위 개념(더 일반·근본)**: [LLM 제공자](#provider)
...
**영문**: Credential Pool · **분류**: [에이전트 코어·대화 루프](README.md#분류content-class) · **최초 등장**: 2025
...
**영문**: Mixture-of-Agents · **분류**: [에이전트 코어·대화 루프](README.md#분류content-class) · **최초 등장**: 2024-06
...
**하위 개념(더 일반·근본)**: [앙상블](#ensemble)

**상위 개념(이를 활용해 만든 개념)**: [제안자/종합자](#proposer-aggregator)

**관련 용어**: [제안자/종합자](#proposer-aggregator) · [Mixture-of-Experts (MoE)](#moe) · [에이전트](#agent)
...
**영문**: Proposer / Aggregator · **분류**: [에이전트 코어·대화 루프](README.md#분류content-class) · **최초 등장**: 2024-06

[MoA](#moa)의 두 역할: 제안자(proposer)들이 초안을 내고, 종합자(aggregator)가 초안들을 읽고 하나의 개선된 답으로 합칩니다.

**하위 개념(더 일반·근본)**: [Mixture-of-Agents (MoA)](#moa)
...
**영문**: Ensemble · **분류**: [에이전트 코어·대화 루프](README.md#분류content-class) · **최초 등장**: 1990년대(기계학습)

여러 모델의 예측을 결합해 단일 모델보다 나은 결과를 얻는 고전적 머신러닝 기법(배깅, 부스팅 등). [MoA](#moa)는 생성 태스크용 앙상블로 볼 수 있습니다.

**상위 개념(이를 활용해 만든 개념)**: [Mixture-of-Agents (MoA)](#moa) · [자기일관성](#self-consistency)

**관련 용어**: [Mixture-of-Agents (MoA)](#moa) · [자기일관성](#self-consistency)

**이 용어를 참조하는 항목**: [LLM 심판](#llm-as-judge)
...
**영문**: Self-Consistency · **분류**: [에이전트 코어·대화 루프](README.md#분류content-class) · **최초 등장**: 2022-03
...
**영문**: LLM-as-a-Judge · **분류**: [에이전트 코어·대화 루프](README.md#분류content-class) · **최초 등장**: 2023-06

강한 LLM에게 후보 답변들을 채점·선택하게 하는 기법. [종합자](#proposer-aggregator) 대신 '심판'을 쓰는 변형이며 평가 벤치마크에서도 널리 쓰입니다.

**하위 개념(더 일반·근본)**: [LLM (대규모 언어 모델)](01_llm_basics.md#llm)

**관련 용어**: [앙상블](#ensemble) · [제안자/종합자](#proposer-aggregator)
...
**영문**: Mixture-of-Experts · **분류**: [에이전트 코어·대화 루프](README.md#분류content-class) · **최초 등장**: 1991·2017(희소 MoE)
...
**하위 개념(더 일반·근본)**: [트랜스포머](01_llm_basics.md#transformer)

**관련 용어**: [Mixture-of-Agents (MoA)](#moa)

**이 용어를 참조하는 항목**: [Mixture-of-Agents (MoA)](#moa) · [트랜스포머](01_llm_basics.md#transformer)
...
**영문**: Subagent / Delegation · **분류**: [에이전트 코어·대화 루프](README.md#분류content-class) · **최초 등장**: 2023
...
**상위 개념(이를 활용해 만든 개념)**: [델리게이션 (작업 위임)](#delegation)
...
**영문**: Delegation · **분류**: [에이전트 코어·대화 루프](README.md#분류content-class) · **최초 등장**: 2023

복잡한 작업을 하위 작업으로 나눠 [서브에이전트](#subagent)에 맡기는 패턴. 컨텍스트를 분리해 메인 대화를 깨끗하게 유지하는 효과도 있습니다. 다중 에이전트 협업에는 [칸반](#kanban)이 사용됩니다.

**하위 개념(더 일반·근본)**: [서브에이전트 / 위임](#subagent)

**상위 개념(이를 활용해 만든 개념)**: [칸반 (다중 에이전트 보드)](#kanban)
...
**영문**: Kanban · **분류**: [에이전트 코어·대화 루프](README.md#분류content-class) · **최초 등장**: 1940년대(도요타)·2025(Hermes)

여러 에이전트가 작업을 주고받는 내구성 있는 작업 보드. 디스패처가 작업을 배분하고 워커 에이전트들이 처리하는 다중 에이전트 협업 계층입니다.

**하위 개념(더 일반·근본)**: [델리게이션 (작업 위임)](#delegation)
...
**영문**: Multi-Agent System · **분류**: [에이전트 코어·대화 루프](README.md#분류content-class) · **최초 등장**: 1980년대(DAI)·2023(LLM)
...
**상위 개념(이를 활용해 만든 개념)**: [오케스트레이션](#orchestration)
...
**영문**: Orchestration · **분류**: [에이전트 코어·대화 루프](README.md#분류content-class) · **최초 등장**: 2023
...
**하위 개념(더 일반·근본)**: [다중 에이전트 시스템](#multi-agent)
...
**영문**: Human-in-the-Loop · **분류**: [에이전트 코어·대화 루프](README.md#분류content-class) · **최초 등장**: 1950년대(제어)
...
**영문**: Autonomy Level · **분류**: [에이전트 코어·대화 루프](README.md#분류content-class) · **최초 등장**: 2023
...
**영문**: Loop Termination · **분류**: [에이전트 코어·대화 루프](README.md#분류content-class) · **최초 등장**: 2025
