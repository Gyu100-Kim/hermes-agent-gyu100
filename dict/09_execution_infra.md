...
**영문**: Execution Environment · **분류**: [실행 환경·인프라](README.md#분류content-class)
...
**상위 개념(더 특수)**: [Daytona 백엔드](#daytona-backend) · [Docker 백엔드](#docker-backend) · [원격 파일 동기화](#file-sync) · [Modal 백엔드](#modal-backend) · [샌드박스 (격리)](#sandbox) · [Singularity/Apptainer 백엔드](#singularity-backend) · [호출별 스폰 모델](#spawn-per-call) · [SSH 백엔드](#ssh-backend)

**관련 용어**: [터미널 도구](03_tool_system.md#terminal-tool) · [호출별 스폰 모델](#spawn-per-call) · [샌드박스 (격리)](#sandbox)

**이 용어를 참조하는 항목**: [터미널 도구](03_tool_system.md#terminal-tool) · [턴 격리](11_design_principles.md#turn-isolation)
...
**영문**: Sandbox / Isolation · **분류**: [실행 환경·인프라](README.md#분류content-class)
...
**하위 개념(더 일반)**: [실행 환경](#execution-environment)

**상위 개념(더 특수)**: [컨테이너](#container) · [VM 격리](#vm-isolation)
...
**영문**: Spawn-per-Call · **분류**: [실행 환경·인프라](README.md#분류content-class)
...
**상위 개념(더 특수)**: [세션 스냅샷 (셸 상태)](#session-snapshot)
...
**영문**: Session Snapshot · **분류**: [실행 환경·인프라](README.md#분류content-class)
...
**하위 개념(더 일반)**: [호출별 스폰 모델](#spawn-per-call)
...
**영문**: Container · **분류**: [실행 환경·인프라](README.md#분류content-class)
...
**하위 개념(더 일반)**: [샌드박스 (격리)](#sandbox)

**상위 개념(더 특수)**: [cgroups](#cgroups) · [컨테이너 이미지](#container-image) · [Docker 백엔드](#docker-backend) · [리눅스 네임스페이스](#namespace) · [루트리스 컨테이너](#rootless) · [Singularity/Apptainer 백엔드](#singularity-backend)
...
**영문**: Linux Namespaces · **분류**: [실행 환경·인프라](README.md#분류content-class)
...
**영문**: Control Groups · **분류**: [실행 환경·인프라](README.md#분류content-class)
...
**영문**: Container Image · **분류**: [실행 환경·인프라](README.md#분류content-class)
...
**영문**: Docker Backend · **분류**: [실행 환경·인프라](README.md#분류content-class)

명령을 Docker 컨테이너 안에서 실행하는 백엔드(`tools/environments/docker.py`). 호스트와 격리된 일회용 작업 공간을 제공합니다.

**하위 개념(더 일반)**: [실행 환경](#execution-environment) · [컨테이너](#container)
...
**영문**: SSH Backend · **분류**: [실행 환경·인프라](README.md#분류content-class)
...
**영문**: Modal Backend · **분류**: [실행 환경·인프라](README.md#분류content-class)

서버리스 컴퓨트 플랫폼 Modal에서 명령을 실행하는 백엔드. 필요할 때만 클라우드 샌드박스를 띄워 쓰고 반납합니다.

**하위 개념(더 일반)**: [실행 환경](#execution-environment) · [서버리스 컴퓨트](#serverless)
...
**영문**: Daytona Backend · **분류**: [실행 환경·인프라](README.md#분류content-class)
...
**영문**: Singularity Backend · **분류**: [실행 환경·인프라](README.md#분류content-class)
...
**영문**: Rootless Container · **분류**: [실행 환경·인프라](README.md#분류content-class)
...
**영문**: VM Isolation · **분류**: [실행 환경·인프라](README.md#분류content-class)
...
**영문**: Serverless Compute · **분류**: [실행 환경·인프라](README.md#분류content-class)

서버를 상시 운영하지 않고 함수/작업 단위로 필요할 때만 실행 환경을 빌려 쓰는 모델. 쓴 만큼만 과금됩니다.

> **예시**: 하루 10분만 필요한 GPU 작업을 위해 서버를 상시 임대하는 대신, Modal에서 10분만 빌려 씁니다.

**상위 개념(더 특수)**: [Modal 백엔드](#modal-backend)

**관련 용어**: [Modal 백엔드](#modal-backend)

**이 용어를 참조하는 항목**: [턴 격리](11_design_principles.md#turn-isolation)
...
**영문**: Remote File Sync · **분류**: [실행 환경·인프라](README.md#분류content-class)
...
**영문**: Reproducibility · **분류**: [실행 환경·인프라](README.md#분류content-class)

같은 입력에서 언제 어디서든 같은 환경·결과가 나오는 성질. [이미지](#container-image), [Nix](#nix), [의존성 고정](#exact-pinning)이 모두 이를 위한 수단입니다.

**상위 개념(더 특수)**: [의존성 정확 고정](#exact-pinning) · [Nix / Flake](#nix)
...
**영문**: Exact Pinning · **분류**: [실행 환경·인프라](README.md#분류content-class)
...
**영문**: Lazy Dependencies · **분류**: [실행 환경·인프라](README.md#분류content-class)
...
**영문**: CI/CD · **분류**: [실행 환경·인프라](README.md#분류content-class)
