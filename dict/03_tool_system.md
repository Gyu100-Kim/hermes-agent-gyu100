...
**영문**: Tool · **분류**: [도구 시스템](README.md#분류content-class) · **최초 등장**: 2023-03
...
**상위 개념(이를 활용해 만든 개념)**: [브라우저 도구](#browser-tool) · [컴퓨터 사용](#computer-use) · [delegate 도구](#delegate-tool) · [핸들러](#handler) · [관리형 도구](11_design_principles.md#managed-tools) · [스킬 도구 (skill_view)](#skill-tool) · [도구 호출 (함수 호출)](02_agent_core.md#tool-calling) · [도구 레지스트리](#tool-registry) · [도구 스키마](#tool-schema) · [툴셋](#toolset) · [비전 도구](#vision-tools) · [음성 모드](#voice-mode)

**관련 용어**: [도구 스키마](#tool-schema) · [핸들러](#handler) · [툴셋](#toolset) · [도구 호출 (함수 호출)](02_agent_core.md#tool-calling)
...
**영문**: Tool Schema · **분류**: [도구 시스템](README.md#분류content-class) · **최초 등장**: 2023-06
...
**하위 개념(더 일반·근본)**: [도구](#tool) · [JSON Schema](#json-schema)
...
**영문**: JSON Schema · **분류**: [도구 시스템](README.md#분류content-class) · **최초 등장**: 2010-01
...
**상위 개념(이를 활용해 만든 개념)**: [구조화 출력](01_llm_basics.md#structured-output) · [도구 스키마](#tool-schema)
...
**영문**: Tool Registry · **분류**: [도구 시스템](README.md#분류content-class) · **최초 등장**: 2025

모든 도구의 등록부(`tools/registry.py`). 도구 이름 → [핸들러](#handler)/스키마/[가용성 검사](#check-fn) 매핑을 보관하며, [디스패치](#dispatch)의 진입점입니다.

**하위 개념(더 일반·근본)**: [도구](#tool)

**상위 개념(이를 활용해 만든 개념)**: [가용성 검사 (check_fn)](#check-fn) · [디스패치](#dispatch) · [자기 등록](#self-registration)
...
**영문**: Self-Registration · **분류**: [도구 시스템](README.md#분류content-class) · **최초 등장**: 2025

각 도구 파일이 import될 때 스스로 `registry.register(...)`를 호출해 등록되는 패턴. 코어를 수정하지 않고 도구를 추가할 수 있게 합니다.

**하위 개념(더 일반·근본)**: [도구 레지스트리](#tool-registry)

**상위 개념(이를 활용해 만든 개념)**: [AST 기반 도구 발견](#ast-discovery)
...
**영문**: AST-based Discovery · **분류**: [도구 시스템](README.md#분류content-class) · **최초 등장**: 2025
...
**하위 개념(더 일반·근본)**: [자기 등록](#self-registration)
...
**영문**: Dispatch · **분류**: [도구 시스템](README.md#분류content-class) · **최초 등장**: 2025
...
**상위 개념(이를 활용해 만든 개념)**: [에러 봉투](#error-envelope) · [결과 정규화](#result-normalization)
...
**영문**: Handler · **분류**: [도구 시스템](README.md#분류content-class) · **최초 등장**: 2025
...
**영문**: Result Normalization · **분류**: [도구 시스템](README.md#분류content-class) · **최초 등장**: 2025

핸들러가 돌려준 다양한 형태(문자열, dict, 예외)의 결과를 모델에 반환 가능한 일관된 형식으로 변환하는 단계. 결과 크기 상한도 여기서 적용됩니다.

**하위 개념(더 일반·근본)**: [디스패치](#dispatch)

**상위 개념(이를 활용해 만든 개념)**: [도구 결과 크기 상한](#max-result-size)
...
**영문**: Error Envelope · **분류**: [도구 시스템](README.md#분류content-class) · **최초 등장**: 2025
...
**영문**: check_fn (Service Gating) · **분류**: [도구 시스템](README.md#분류content-class) · **최초 등장**: 2025
...
**영문**: Toolset · **분류**: [도구 시스템](README.md#분류content-class) · **최초 등장**: 2025

도구들의 논리적 묶음(파일, 브라우저, 스킬 등). 세션·작업별로 활성화할 툴셋을 선택할 수 있습니다.

**하위 개념(더 일반·근본)**: [도구](#tool)

**상위 개념(이를 활용해 만든 개념)**: [코어 도구](#core-tools)
...
**영문**: Core Tools (_HERMES_CORE_TOOLS) · **분류**: [도구 시스템](README.md#분류content-class) · **최초 등장**: 2025
...
**하위 개념(더 일반·근본)**: [툴셋](#toolset)

**상위 개념(이를 활용해 만든 개념)**: [파일 도구](#file-tools) · [터미널 도구](#terminal-tool) · [웹 검색 도구](#web-search-tool)
...
**영문**: Terminal Tool · **분류**: [도구 시스템](README.md#분류content-class) · **최초 등장**: 2025
...
**하위 개념(더 일반·근본)**: [코어 도구](#core-tools) · [실행 환경](09_execution_infra.md#execution-environment)

**상위 개념(이를 활용해 만든 개념)**: [명령 승인](10_security.md#command-approval)

**관련 용어**: [명령 승인](10_security.md#command-approval)
...
**영문**: File Tools · **분류**: [도구 시스템](README.md#분류content-class) · **최초 등장**: 2025

파일 읽기/쓰기/편집 도구군. 에이전트가 코드베이스를 읽고 수정하는 기본 수단입니다.

**하위 개념(더 일반·근본)**: [코어 도구](#core-tools)
...
**영문**: Browser Tool · **분류**: [도구 시스템](README.md#분류content-class) · **최초 등장**: 2025
...
**하위 개념(더 일반·근본)**: [도구](#tool) · [CDP (크롬 개발자도구 프로토콜)](08_protocols.md#cdp)

**상위 개념(이를 활용해 만든 개념)**: [browser_cdp (CDP 탈출구)](#browser-cdp-tool) · [클라우드 브라우저](08_protocols.md#cloud-browser)

**관련 용어**: [접근성 트리](08_protocols.md#accessibility-tree) · [클라우드 브라우저](08_protocols.md#cloud-browser)

**이 용어를 참조하는 항목**: [접근성 트리](08_protocols.md#accessibility-tree) · [CDP (크롬 개발자도구 프로토콜)](08_protocols.md#cdp) · [컴퓨터 사용](#computer-use)
...
**영문**: browser_cdp Tool · **분류**: [도구 시스템](README.md#분류content-class) · **최초 등장**: 2025
...
**하위 개념(더 일반·근본)**: [브라우저 도구](#browser-tool)
...
**영문**: Vision Tools · **분류**: [도구 시스템](README.md#분류content-class) · **최초 등장**: 2025

이미지를 분석하는 도구군. 비전 요청을 메인 대화 모델과 독립적으로 전문 모델에 라우팅합니다.

**하위 개념(더 일반·근본)**: [도구](#tool) · [비전 모델 (VLM)](01_llm_basics.md#vision-model)
...
**영문**: Web Search Tool · **분류**: [도구 시스템](README.md#분류content-class) · **최초 등장**: 2025
...
**영문**: Delegate Tool · **분류**: [도구 시스템](README.md#분류content-class) · **최초 등장**: 2025
...
**영문**: Skill Tools · **분류**: [도구 시스템](README.md#분류content-class) · **최초 등장**: 2025
...
**영문**: Computer Use · **분류**: [도구 시스템](README.md#분류content-class) · **최초 등장**: 2024-10
...
**영문**: Voice Mode · **분류**: [도구 시스템](README.md#분류content-class) · **최초 등장**: 2024-05

음성 입출력(STT/TTS)으로 에이전트와 대화하는 모드.

**하위 개념(더 일반·근본)**: [도구](#tool) · [음성 인식 (STT)](01_llm_basics.md#stt) · [음성 합성 (TTS)](01_llm_basics.md#tts)
...
**영문**: Max Result Size · **분류**: [도구 시스템](README.md#분류content-class) · **최초 등장**: 2025

도구 결과가 [컨텍스트](01_llm_basics.md#context-window)를 폭식하지 않도록 문자 수 상한을 두고 초과분을 자르는 장치. 도구별로 상한을 다르게 설정할 수 있습니다.

**하위 개념(더 일반·근본)**: [결과 정규화](#result-normalization)
