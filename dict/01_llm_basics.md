...
**영문**: Large Language Model · **분류**: [LLM 기초](README.md#분류content-class)
...
**상위 개념(더 특수)**: [보조 모델](#auxiliary-model) · [벤치마크 (평가)](13_model_learning.md#benchmark) · [컨텍스트 윈도우](#context-window) · [임베딩](#embedding) · [그라운딩](#grounding) · [환각](#hallucination) · [추론 (서빙)](#inference) · [멀티모달](#multimodal) · [오픈 웨이트 모델](13_model_learning.md#open-weights) · [사전학습](13_model_learning.md#pretraining) · [프롬프트](#prompt) · [LLM 제공자](02_agent_core.md#provider) · [추론 모델 / 사고 과정](#reasoning-model) · [샘플링 / 온도](#sampling) · [구조화 출력](#structured-output) · [토큰](#token) · [트랜스포머](#transformer)
...
**영문**: Token · **분류**: [LLM 기초](README.md#분류content-class)
...
**영문**: Context Window · **분류**: [LLM 기초](README.md#분류content-class)
...
**상위 개념(더 특수)**: [중간 소실 현상](04_prompt_context.md#lost-in-the-middle)
...
**영문**: System Prompt · **분류**: [LLM 기초](README.md#분류content-class)
...
**하위 개념(더 일반)**: [프롬프트](#prompt)

**상위 개념(더 특수)**: [시스템 프롬프트 3계층](04_prompt_context.md#system-prompt-tiers)

**관련 용어**: [프롬프트 캐싱](#prompt-caching) · [SOUL.md](04_prompt_context.md#soul-md) · [시스템 프롬프트 3계층](04_prompt_context.md#system-prompt-tiers)

**이 용어를 참조하는 항목**: [컨텍스트 엔지니어링](04_prompt_context.md#context-engineering) · [지시 튜닝](13_model_learning.md#instruction-tuning) · [프롬프트](#prompt)
...
**영문**: Prompt · **분류**: [LLM 기초](README.md#분류content-class)

모델에 입력으로 주는 텍스트 전체. [시스템 프롬프트](#system-prompt) + 대화 이력 + 도구 결과가 모두 프롬프트에 포함되어 매 호출 전송됩니다.

**하위 개념(더 일반)**: [LLM (대규모 언어 모델)](#llm)

**상위 개념(더 특수)**: [프롬프트 캐싱](#prompt-caching) · [시스템 프롬프트](#system-prompt)
...
**영문**: Prompt Caching · **분류**: [LLM 기초](README.md#분류content-class)
...
**상위 개념(더 특수)**: [제공자별 캐시 구현](#context-caching-provider)
...
**영문**: Sampling / Temperature · **분류**: [LLM 기초](README.md#분류content-class)
...
**이 용어를 참조하는 항목**: [제약 디코딩](#constrained-decoding) · [자기일관성](02_agent_core.md#self-consistency)
...
**영문**: Reasoning Model / Chain-of-Thought · **분류**: [LLM 기초](README.md#분류content-class)
...
**영문**: Hallucination · **분류**: [LLM 기초](README.md#분류content-class)
...
**영문**: Grounding · **분류**: [LLM 기초](README.md#분류content-class)
...
**영문**: Auxiliary Model · **분류**: [LLM 기초](README.md#분류content-class)
...
**영문**: Embedding · **분류**: [LLM 기초](README.md#분류content-class)
...
**영문**: Inference / Serving · **분류**: [LLM 기초](README.md#분류content-class)
...
**상위 개념(더 특수)**: [지연 시간](#latency) · [양자화](13_model_learning.md#quantization) · [레이트리밋](#rate-limit) · [응답 스트리밍](#streaming)
...
**영문**: Latency · **분류**: [LLM 기초](README.md#분류content-class)
...
**영문**: Streaming · **분류**: [LLM 기초](README.md#분류content-class)
...
**이 용어를 참조하는 항목**: [추론 (서빙)](#inference) · [지연 시간](#latency)
...
**영문**: Rate Limit · **분류**: [LLM 기초](README.md#분류content-class)
...
**영문**: Exponential Backoff · **분류**: [LLM 기초](README.md#분류content-class)
...
**영문**: Structured Output · **분류**: [LLM 기초](README.md#분류content-class)
...
**상위 개념(더 특수)**: [제약 디코딩](#constrained-decoding)

**관련 용어**: [JSON Schema](03_tool_system.md#json-schema) · [도구 호출 (함수 호출)](02_agent_core.md#tool-calling) · [제약 디코딩](#constrained-decoding)
...
**영문**: Constrained Decoding · **분류**: [LLM 기초](README.md#분류content-class)
...
**하위 개념(더 일반)**: [구조화 출력](#structured-output)

**관련 용어**: [샘플링 / 온도](#sampling)
...
**영문**: Provider Cache Implementations · **분류**: [LLM 기초](README.md#분류content-class)
...
**하위 개념(더 일반)**: [프롬프트 캐싱](#prompt-caching)
...
**영문**: Multimodal · **분류**: [LLM 기초](README.md#분류content-class)
...
**상위 개념(더 특수)**: [음성 인식 (STT)](#stt) · [음성 합성 (TTS)](#tts) · [비전 모델 (VLM)](#vision-model)
...
**영문**: Vision-Language Model · **분류**: [LLM 기초](README.md#분류content-class)
...
**영문**: Speech-to-Text · **분류**: [LLM 기초](README.md#분류content-class)
...
**영문**: Text-to-Speech · **분류**: [LLM 기초](README.md#분류content-class)
