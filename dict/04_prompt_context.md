...
이 문서는 분류(Content Class) **프롬프트·컨텍스트** 에 속한 용어 15개를 다룹니다.
...
**영문**: Context Engineering · **분류**: [프롬프트·컨텍스트](README.md#분류content-class) · **최초 등장**: 2024
...
**하위 개념(더 일반·근본)**: [프롬프트](01_llm_basics.md#prompt) · [컨텍스트 윈도우](01_llm_basics.md#context-window)

**상위 개념(이를 활용해 만든 개념)**: [컨텍스트 압축](#context-compression) · [컨텍스트 엔진](#context-engine) · [점진적 공개](05_memory_self_improvement.md#progressive-disclosure)
...
**영문**: System Prompt Tiers · **분류**: [프롬프트·컨텍스트](README.md#분류content-class) · **최초 등장**: 2025
...
**하위 개념(더 일반·근본)**: [시스템 프롬프트](01_llm_basics.md#system-prompt)

**상위 개념(이를 활용해 만든 개념)**: [컨텍스트 파일 (AGENTS.md)](#context-file) · [SOUL.md](#soul-md)
...
**영문**: SOUL.md · **분류**: [프롬프트·컨텍스트](README.md#분류content-class) · **최초 등장**: 2025

에이전트의 정체성과 행동 원칙을 담은 마크다운 파일. 시스템 프롬프트의 stable 계층에 포함되어 에이전트의 '영혼' 역할을 합니다.

> **예시**: "너는 간결하게 답하고, 확실하지 않으면 모른다고 말한다" 같은 정체성 규칙이 SOUL.md에 담깁니다.

**하위 개념(더 일반·근본)**: [시스템 프롬프트 3계층](#system-prompt-tiers) · [페르소나](#persona)

**이 용어를 참조하는 항목**: [컨텍스트 파일 (AGENTS.md)](#context-file) · [페르소나](#persona) · [시스템 프롬프트](01_llm_basics.md#system-prompt) · [시스템 프롬프트 3계층](#system-prompt-tiers)
...
**영문**: Persona · **분류**: [프롬프트·컨텍스트](README.md#분류content-class) · **최초 등장**: 2022

에이전트가 일관되게 유지하는 성격·말투·가치관. [SOUL.md](#soul-md)로 정의되며 프로필별로 다르게 설정할 수 있습니다.

**하위 개념(더 일반·근본)**: [프롬프트](01_llm_basics.md#prompt)

**상위 개념(이를 활용해 만든 개념)**: [SOUL.md](#soul-md)

**관련 용어**: [SOUL.md](#soul-md) · [시스템 프롬프트](01_llm_basics.md#system-prompt)
...
**영문**: Context Compression · **분류**: [프롬프트·컨텍스트](README.md#분류content-class) · **최초 등장**: 2023-10
...
> **예시**: 10만 토큰 대화에서 중간 7만 토큰을 2천 토큰 요약으로 대체해, 대화를 계속할 여유를 만드는 것이 압축입니다.

**하위 개념(더 일반·근본)**: [컨텍스트 엔지니어링](#context-engineering)

**상위 개념(이를 활용해 만든 개념)**: [압축 쿨다운](#compression-cooldown) · [압축 계보](#compression-lineage) · [머리/꼬리 보호](#head-tail-protection) · [요약 기반 압축](#summarization)
...
**영문**: Summarization · **분류**: [프롬프트·컨텍스트](README.md#분류content-class) · **최초 등장**: 2019(뉴럴 요약)

오래된 대화 구간을 LLM으로 요약해 짧은 텍스트로 대체하는 압축 방식. 정보 손실과 토큰 회수 사이의 균형이 핵심입니다.

**하위 개념(더 일반·근본)**: [컨텍스트 압축](#context-compression)

**상위 개념(이를 활용해 만든 개념)**: [구조화 요약](#structured-summary) · [도구 출력 가지치기](#tool-output-pruning)
...
**영문**: Structured Summary · **분류**: [프롬프트·컨텍스트](README.md#분류content-class) · **최초 등장**: 2025

자유 서술 대신 정해진 템플릿(해결된 질문/미해결 질문/핵심 결정 등)으로 요약을 생성하는 기법. 후속 대화에 필요한 정보가 누락되는 것을 줄입니다.

> **예시**: 자유 요약 대신 "해결됨: DB 마이그레이션 / 미해결: 테스트 2개 실패 / 결정: pytest로 통일" 형식으로 남기는 것입니다.

**하위 개념(더 일반·근본)**: [요약 기반 압축](#summarization)
...
**영문**: Head/Tail Protection · **분류**: [프롬프트·컨텍스트](README.md#분류content-class) · **최초 등장**: 2023-07

압축 시 대화의 머리(초기 지시·목표)와 꼬리(최근 맥락)는 원문 그대로 두고 중간만 요약하는 전략. [중간 소실](#lost-in-the-middle) 연구와도 일치하는 설계입니다.

> **예시**: "처음의 작업 지시(머리)"와 "방금 나눈 대화(꼬리)"는 원문 유지, 그 사이의 시행착오 기록만 요약 대상이 됩니다.

**하위 개념(더 일반·근본)**: [컨텍스트 압축](#context-compression)

**상위 개념(이를 활용해 만든 개념)**: [토큰 예산 기반 꼬리 보호](#token-budget-tail)
...
**영문**: Token-Budget Tail Protection · **분류**: [프롬프트·컨텍스트](README.md#분류content-class) · **최초 등장**: 2025

'최근 N개 메시지'가 아니라 '꼬리에 남길 토큰 예산'으로 보호 범위를 정하는 방식. 메시지 길이 편차에 강건합니다.

**하위 개념(더 일반·근본)**: [머리/꼬리 보호](#head-tail-protection)
...
**영문**: Tool Output Pruning · **분류**: [프롬프트·컨텍스트](README.md#분류content-class) · **최초 등장**: 2025
...
**영문**: Lost in the Middle · **분류**: [프롬프트·컨텍스트](README.md#분류content-class) · **최초 등장**: 2023-07

긴 컨텍스트에서 모델이 처음과 끝은 잘 기억하지만 중간 정보는 잘 놓친다는 2023년 연구 결과. '중간부터 요약하라'는 [머리/꼬리 보호](#head-tail-protection) 설계의 근거입니다.

> **예시**: 20개 문서를 컨텍스트에 넣고 질문하면, 정답이 10번째 문서에 있을 때 정확도가 가장 낮게 나옵니다(U자형 곡선).

**하위 개념(더 일반·근본)**: [컨텍스트 윈도우](01_llm_basics.md#context-window)
...
**영문**: Compression Lineage (parent_session_id) · **분류**: [프롬프트·컨텍스트](README.md#분류content-class) · **최초 등장**: 2025
...
**영문**: Context Engine · **분류**: [프롬프트·컨텍스트](README.md#분류content-class) · **최초 등장**: 2025
...
**영문**: Context Files · **분류**: [프롬프트·컨텍스트](README.md#분류content-class) · **최초 등장**: 2025

작업 디렉토리의 AGENTS.md 등 저장소별 지침 파일. 시스템 프롬프트의 context 계층에 삽입되어 저장소 규칙을 에이전트에게 알립니다.

**하위 개념(더 일반·근본)**: [시스템 프롬프트 3계층](#system-prompt-tiers)
...
**영문**: Compression Cooldown · **분류**: [프롬프트·컨텍스트](README.md#분류content-class) · **최초 등장**: 2025
