...
**영문**: SessionDB · **분류**: [상태·영속성·검색](README.md#분류content-class) · **최초 등장**: 2025

Hermes의 상태 저장소 클래스(`hermes_state.py`). [SQLite](#sqlite) 파일 하나에 세션·메시지·크론 작업 등을 저장하며, [WAL](#wal) 모드와 [FTS5](#fts5) 검색을 사용합니다.

**하위 개념(더 일반·근본)**: [SQLite](#sqlite)

**상위 개념(이를 활용해 만든 개념)**: [세션](#session)

**관련 용어**: [세션](#session) · [WAL 모드](#wal) · [FTS5](#fts5)

**이 용어를 참조하는 항목**: [압축 계보](04_prompt_context.md#compression-lineage) · [크론 (예약 작업)](12_subsystems.md#cron) · [일화 기억](05_memory_self_improvement.md#episodic-memory) · [WAL 모드](#wal)
...
**영문**: SQLite · **분류**: [상태·영속성·검색](README.md#분류content-class) · **최초 등장**: 2000-08
...
**상위 개념(이를 활용해 만든 개념)**: [FTS5](#fts5) · [SessionDB](#sessiondb) · [WAL 모드](#wal)

**관련 용어**: [WAL 모드](#wal) · [FTS5](#fts5)

**이 용어를 참조하는 항목**: [FTS 동기화 트리거](#fts-trigger)
...
**영문**: Write-Ahead Logging · **분류**: [상태·영속성·검색](README.md#분류content-class) · **최초 등장**: 2010-07(SQLite 3.7)
...
**영문**: Full-Text Search · **분류**: [상태·영속성·검색](README.md#분류content-class) · **최초 등장**: 1960년대(IR)

문서 안의 단어를 미리 색인해 두고 질의어가 포함된 문서를 빠르게 찾는 기술. LIKE '%...%' 스캔과 달리 문서가 많아도 빠릅니다.

**하위 개념(더 일반·근본)**: [역색인](#inverted-index)

**상위 개념(이를 활용해 만든 개념)**: [FTS5](#fts5) · [토크나이저 (검색)](#tokenizer)

**관련 용어**: [FTS5](#fts5) · [검색 증강 (RAG)](#retrieval)

**이 용어를 참조하는 항목**: [역색인](#inverted-index) · [검색 증강 (RAG)](#retrieval)
...
**영문**: FTS5 · **분류**: [상태·영속성·검색](README.md#분류content-class) · **최초 등장**: 2015-07
...
**하위 개념(더 일반·근본)**: [전문 검색 (FTS)](#fts) · [SQLite](#sqlite)

**상위 개념(이를 활용해 만든 개념)**: [외부 콘텐츠 테이블](#external-content) · [세션 검색](#session-search)

**관련 용어**: [BM25](#bm25) · [토크나이저 (검색)](#tokenizer) · [외부 콘텐츠 테이블](#external-content) · [트라이그램 토크나이저](#trigram)

**이 용어를 참조하는 항목**: [BM25](#bm25) · [전문 검색 (FTS)](#fts) · [SessionDB](#sessiondb) · [SQLite](#sqlite) · [토크나이저 (검색)](#tokenizer) · [unicode61 토크나이저](#unicode61)
...
**영문**: Inverted Index · **분류**: [상태·영속성·검색](README.md#분류content-class) · **최초 등장**: 1950년대(IR)
...
**상위 개념(이를 활용해 만든 개념)**: [전문 검색 (FTS)](#fts)

**관련 용어**: [토크나이저 (검색)](#tokenizer) · [전문 검색 (FTS)](#fts)

**이 용어를 참조하는 항목**: [TF-IDF](#tf-idf)
...
**영문**: Tokenizer (FTS) · **분류**: [상태·영속성·검색](README.md#분류content-class) · **최초 등장**: 1960년대(IR)
...
**하위 개념(더 일반·근본)**: [전문 검색 (FTS)](#fts)

**상위 개념(이를 활용해 만든 개념)**: [트라이그램 토크나이저](#trigram) · [unicode61 토크나이저](#unicode61)
...
**영문**: unicode61 · **분류**: [상태·영속성·검색](README.md#분류content-class) · **최초 등장**: 2012(SQLite FTS4)

유니코드 공백·구두점 기준으로 단어를 나누는 [FTS5](#fts5) 기본 토크나이저. 띄어쓰기가 있는 언어(영어·한국어 어절)에 적합하지만 중국어·일본어처럼 띄어쓰기가 없는 언어에는 부적합합니다.

**하위 개념(더 일반·근본)**: [토크나이저 (검색)](#tokenizer)
...
**영문**: Trigram · **분류**: [상태·영속성·검색](README.md#분류content-class) · **최초 등장**: 1994(n-gram IR)·2020(SQLite 3.34)
...
**영문**: BM25 (Okapi) · **분류**: [상태·영속성·검색](README.md#분류content-class) · **최초 등장**: 1994
...
**하위 개념(더 일반·근본)**: [TF-IDF](#tf-idf)

**상위 개념(이를 활용해 만든 개념)**: [하이브리드 검색](#hybrid-search)

**관련 용어**: [FTS5](#fts5)

**이 용어를 참조하는 항목**: [FTS5](#fts5) · [TF-IDF](#tf-idf)
...
**영문**: TF-IDF · **분류**: [상태·영속성·검색](README.md#분류content-class) · **최초 등장**: 1972

단어 빈도(TF)와 역문서빈도(IDF)를 곱해 단어의 중요도를 재는 고전 정보검색 공식. [BM25](#bm25)의 전신입니다.

**상위 개념(이를 활용해 만든 개념)**: [BM25](#bm25)

**관련 용어**: [BM25](#bm25) · [역색인](#inverted-index)
...
**영문**: External-Content FTS Table · **분류**: [상태·영속성·검색](README.md#분류content-class) · **최초 등장**: 2012(FTS4)

원문을 FTS 테이블에 중복 저장하지 않고 원본 테이블을 참조만 하는 [FTS5](#fts5) 구성. 저장 공간을 절약하는 대신 [트리거](#fts-trigger)로 색인을 동기화해야 합니다.

**하위 개념(더 일반·근본)**: [FTS5](#fts5)

**상위 개념(이를 활용해 만든 개념)**: [FTS 동기화 트리거](#fts-trigger)
...
**영문**: FTS Sync Triggers · **분류**: [상태·영속성·검색](README.md#분류content-class) · **최초 등장**: 2012

messages 테이블의 INSERT/UPDATE/DELETE 시 자동으로 FTS 색인을 갱신하는 [SQLite](#sqlite) 트리거들. 애플리케이션 코드가 색인을 잊어버리는 실수를 원천 차단합니다.

**하위 개념(더 일반·근본)**: [외부 콘텐츠 테이블](#external-content)
...
**영문**: Session Search · **분류**: [상태·영속성·검색](README.md#분류content-class) · **최초 등장**: 2025
...
**영문**: Retrieval / RAG · **분류**: [상태·영속성·검색](README.md#분류content-class) · **최초 등장**: 2020-05(RAG)
...
**상위 개념(이를 활용해 만든 개념)**: [에이전트형 RAG](#agentic-rag) · [벡터 검색](#vector-search)

**관련 용어**: [전문 검색 (FTS)](#fts) · [벡터 검색](#vector-search) · [에이전트형 RAG](#agentic-rag)

**이 용어를 참조하는 항목**: [전문 검색 (FTS)](#fts) · [그라운딩](01_llm_basics.md#grounding) · [하이브리드 검색](#hybrid-search)
...
**영문**: Vector Search · **분류**: [상태·영속성·검색](README.md#분류content-class) · **최초 등장**: 2017(HNSW 상용화)
...
**하위 개념(더 일반·근본)**: [검색 증강 (RAG)](#retrieval) · [임베딩](01_llm_basics.md#embedding)

**상위 개념(이를 활용해 만든 개념)**: [하이브리드 검색](#hybrid-search)

**관련 용어**: [하이브리드 검색](#hybrid-search)

**이 용어를 참조하는 항목**: [임베딩](01_llm_basics.md#embedding) · [검색 증강 (RAG)](#retrieval)
...
**영문**: Hybrid Search · **분류**: [상태·영속성·검색](README.md#분류content-class) · **최초 등장**: 2021

키워드 검색([BM25](#bm25))과 [벡터 검색](#vector-search)의 결과를 결합하는 방식. Hermes는 의존성 최소화를 위해 키워드 검색만 내장하고, 벡터 검색은 외부 확장에 맡깁니다.

**하위 개념(더 일반·근본)**: [벡터 검색](#vector-search) · [BM25](#bm25)

**관련 용어**: [검색 증강 (RAG)](#retrieval)
...
**영문**: Agentic RAG · **분류**: [상태·영속성·검색](README.md#분류content-class) · **최초 등장**: 2024

파이프라인이 항상 검색을 수행하는 고전 RAG와 달리, 에이전트가 필요하다고 판단할 때 검색 도구를 호출하는 방식. Hermes의 [세션 검색](#session-search)이 이 패턴입니다.

**하위 개념(더 일반·근본)**: [검색 증강 (RAG)](#retrieval) · [에이전트](02_agent_core.md#agent)
