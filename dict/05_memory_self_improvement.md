...
**영문**: Self-Improving Agent · **분류**: [메모리·자기개선](README.md#분류content-class)
...
**상위 개념(더 특수)**: [큐레이터](#curator) · [메모리 (에이전트 기억)](#memory) · [자기 성찰](#self-reflection)
...
**영문**: Agent Memory · **분류**: [메모리·자기개선](README.md#분류content-class)
...
**하위 개념(더 일반)**: [자기개선 에이전트](#self-improving-agent)

**상위 개념(더 특수)**: [일화 기억](#episodic-memory) · [메모리 제공자](#memory-provider) · [절차 기억](#procedural-memory) · [의미 기억](#semantic-memory) · [사용자 모델링](#user-modeling)
...
**영문**: Procedural Memory · **분류**: [메모리·자기개선](README.md#분류content-class)

'어떻게 하는가'에 대한 기억. Hermes에서는 [스킬](#skill)(작업 절차 문서)로 구현됩니다.

**하위 개념(더 일반)**: [메모리 (에이전트 기억)](#memory)
...
**영문**: Semantic Memory · **분류**: [메모리·자기개선](README.md#분류content-class)

'무엇이 사실인가'에 대한 기억(사용자 선호, 프로젝트 지식). [메모리 도구](#memory-tool)와 [메모리 제공자](#memory-provider)가 담당합니다.

**하위 개념(더 일반)**: [메모리 (에이전트 기억)](#memory)

**상위 개념(더 특수)**: [메모리 도구](#memory-tool)
...
**영문**: Episodic Memory · **분류**: [메모리·자기개선](README.md#분류content-class)
...
**영문**: Skill · **분류**: [메모리·자기개선](README.md#분류content-class)
...
**하위 개념(더 일반)**: [절차 기억](#procedural-memory)

**상위 개념(더 특수)**: [옵션 스킬](#optional-skills) · [SKILL.md](#skill-md) · [스킬 허브](#skills-hub)
...
**영문**: SKILL.md · **분류**: [메모리·자기개선](README.md#분류content-class)

스킬의 본문 문서. [프런트매터](#frontmatter)(이름·설명·트리거 조건)와 단계별 절차·주의사항으로 구성됩니다.

**하위 개념(더 일반)**: [스킬](#skill)

**상위 개념(더 특수)**: [프런트매터](#frontmatter)
...
**영문**: Frontmatter · **분류**: [메모리·자기개선](README.md#분류content-class)
...
**영문**: Progressive Disclosure · **분류**: [메모리·자기개선](README.md#분류content-class)
...
**하위 개념(더 일반)**: [컨텍스트 엔지니어링](04_prompt_context.md#context-engineering)
...
**영문**: Curator · **분류**: [메모리·자기개선](README.md#분류content-class)
...
**상위 개념(더 특수)**: [보관 (비파괴 정리)](#archive) · [스킬 고정](#pinning)
...
**영문**: Archive (Non-Destructive) · **분류**: [메모리·자기개선](README.md#분류content-class)

스킬을 지우는 대신 보관 폴더로 옮기는 정책. 자동화된 정리가 잘못돼도 복구할 수 있게 하는 안전장치입니다.

**하위 개념(더 일반)**: [큐레이터](#curator)
...
**영문**: Skill Pinning · **분류**: [메모리·자기개선](README.md#분류content-class)
...
**영문**: Skills Hub · **분류**: [메모리·자기개선](README.md#분류content-class)
...
**영문**: Self-Reflection · **분류**: [메모리·자기개선](README.md#분류content-class)
...
**상위 개념(더 특수)**: [학습 프롬프트](#learn-prompt)
...
**영문**: Learn Prompt · **분류**: [메모리·자기개선](README.md#분류content-class)

대화 경험에서 재사용 가능한 절차를 추출해 [스킬](#skill)로 저장하도록 유도하는 프롬프트(`agent/learn_prompt.py`).

**하위 개념(더 일반)**: [자기 성찰](#self-reflection)
...
**영문**: Memory Tool · **분류**: [메모리·자기개선](README.md#분류content-class)

에이전트가 명시적으로 사실을 저장/조회하는 도구. 문자 수 기반 한도로 모델 독립성을 유지합니다.

**하위 개념(더 일반)**: [의미 기억](#semantic-memory)
...
**영문**: Honcho · **분류**: [메모리·자기개선](README.md#분류content-class)
...
**상위 개념(더 특수)**: [변증법적 API](#dialectic-api) · [마음 이론](#theory-of-mind)
...
**영문**: Theory of Mind · **분류**: [메모리·자기개선](README.md#분류content-class)
...
**영문**: User Modeling · **분류**: [메모리·자기개선](README.md#분류content-class)
...
**영문**: Optional Skills · **분류**: [메모리·자기개선](README.md#분류content-class)
