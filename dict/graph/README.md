...
노드 속성: `id`(고유 식별자), `name_ko`, `name_en`, `category`, `origin`(개념 최초 등장 연월 — 상위/하위 판별의 **참고** 속성. 상위 용어가 항상 늦게 생기는 것은 아님), `definition`(예시 포함 평문).

### 엣지(관계)

| 타입 | 방향 | 의미 |
|---|---|---|
| `UPPER_OF` | (더 특수한 개념) → (더 일반적인 개념) | **계층 엣지.** source는 target의 **상위 개념**입니다. 판별 기준: 개념 B가 개념 A를 **활용해 만들어졌으면** B가 상위, A가 하위. 예: `(트랜스포머)-[:UPPER_OF]->(어텐션)`, `(LLM)-[:UPPER_OF]->(트랜스포머)`, `(LoRA)-[:UPPER_OF]->(PEFT)`, `(PEFT)-[:UPPER_OF]->(파인튜닝)`, `(시스템 프롬프트)-[:UPPER_OF]->(프롬프트)` |
